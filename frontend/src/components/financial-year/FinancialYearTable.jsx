...
const FinancialYearTable = ({ financialYears }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-primary-600 shadow-md shadow-primary-600/40 overflow-hidden">
      <h3 className="text-lg font-semibold p-6 border-b border-gray-200 dark:border-slate-700">
          ...
          <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
            {financialYears.map((fy) => (
              <tr key={fy.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getPeriodBadge(fy.period_status)}`}
                  >
                    {fy.period_status
                      ? fy.period_status.charAt(0).toUpperCase() + fy.period_status.slice(1)
                      : 'Unknown'}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
