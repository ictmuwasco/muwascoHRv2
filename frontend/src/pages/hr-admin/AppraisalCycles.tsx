...
import {
  ...
  type AppraisalCycle,
  type FinancialYearRef,
} from '../../api/services/appraisalCycleService';
...
export default function AppraisalCycles() {
  ...
  const confirmDelete = (row: AppraisalCycle) => {
    if (
      !window.confirm(
        `Delete "${row.name}"? Cycles still referenced by workplan activities are protected.`,
      )
    )
      return;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select
          className="rounded-md border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-gray-700 dark:text-gray-200"
          value={fyFilter}
        ...
        <span className="text-xs text-gray-400">
          {cycles.length} cycle{cycles.length === 1 ? '' : 's'} · every cycle belongs to one
          financial year
        </span>
      </div>
      ...
      >
        {loading ? (
          ...
          <p className="text-center text-gray-500 py-8">
            ...
            {canManage ? ' Click "Add Cycle" to create Q1–Q4 for a financial year.' : ''}
          </p>
        ) : (
          <div className="space-y-6">
                        ...
                        <tr key={c.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30">
                          ...
                          {canManage && (
                            <td className="px-4 py-3 text-right whitespace-nowrap">
                              <button
                                onClick={() => setForm({ open: true, mode: 'edit', record: c })}
                                title="Edit cycle"
...
function groupCyclesByFy(
  cycles: AppraisalCycle[],
  fys: FinancialYearRef[],
  fyFilter: string,
): FyGroup[] {
  const groups = new Map<string, FyGroup>();
  for (const fy of fys) {
    groups.set(String(fy.id), {
      fyId: fy.id,
      fyName: fy.year_name,
...
function CycleFormModal({ mode, record, financialYears, onClose, onSubmit }: CycleFormModalProps) {
  ...
  const [fyId, setFyId] = useState<string>(
    record?.financial_year_id ? String(record.financial_year_id) : '',
  );
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (
      selectedFy &&
      (startDate < selectedFy.start_date.slice(0, 10) || endDate > selectedFy.end_date.slice(0, 10))
    ) {
      return setErr(
        `The cycle dates must fall inside the ${selectedFy.year_name} financial year (${selectedFy.start_date.slice(0, 10)} to ${selectedFy.end_date.slice(0, 10)}).`,
      );
    }
    setErr('');
    onSubmit(
      ...
      <form onSubmit={submit} className="space-y-4">
        <div>
          ...
          {selectedFy && (
            <p className="text-xs text-gray-400 mt-1">
              Cycle period must be within {selectedFy.start_date.slice(0, 10)} –{' '}
              {selectedFy.end_date.slice(0, 10)}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Q1 2026/2027 Appraisal"
            required
          />
        </div>
