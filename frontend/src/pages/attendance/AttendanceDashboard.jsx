...
const AttendanceDashboard = () => {
  ...
  const date =
    preset === 'today' ? todayStr() : preset === 'yesterday' ? yesterdayStr() : customDate;
        ...
        if (requestId === requestIdRef.current) {
          setError(
            err?.response?.data?.message ||
              err?.message ||
              'Failed to load the attendance dashboard.',
          );
        }
      } finally {
        if (requestId === requestIdRef.current) {
  ...
  const openDetail = async (employee) => {
    ...
    try {
      ...
      const response = await api.get('/attendance/hr-employee-history', {
        params: {
          employee_id: employee.employee_db_id,
          start_date: start,
          end_date: end,
          limit: 30,
        },
  ...
  const absentCount =
    (summary?.absent ?? 0) + ((context?.not_clocked_in_enabled ? summary?.not_clocked_in : 0) || 0);

  const statusOptions = [
    { value: '', label: 'All Statuses' },
    { value: 'PRESENT', label: 'Present' },
    ...(context?.not_clocked_in_enabled
  ...
  const tableColumnsBase = {
    ...
    department: {
      ...
      render: (value, row) => (
        <div>
          ...
          {row.section && (
            <div className="text-xs text-gray-500 dark:text-gray-400">{row.section}</div>
          )}
        </div>
      ),
    },
    ...
    clockIn: {
      ...
      render: (value, row) =>
        value ? (
          <span className={row.is_late ? 'text-yellow-600 dark:text-yellow-400 font-medium' : ''}>
            {value}
          </span>
        ) : (
    ...
    status: {
      ...
      label: 'Status',
      render: (_, row) => (
        <Badge variant={STATUS_META[row.status]?.variant || 'default'}>
      ...
      : activeStatusTab === 'ABSENT'
        ...
        : activeStatusTab === 'ON_LEAVE'
          ...
          : {
              ...
              subtitle: data
                ? `${pagination?.total ?? 0} record(s) matching the current filters`
                : undefined,
            };
  ...
  return (
    <div className="space-y-6">
      ...
      {context?.is_holiday && (
        <div
          className="flex items-start gap-3 rounded-lg border border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20 p-4"
          role="status"
        >
          <CalendarOff className="h-5 w-5 text-blue-600 dark:text-blue-400 mt-0.5" />
          <div>
            <p className="font-medium text-blue-900 dark:text-blue-200">
              Public Holiday — {context.holiday_name}
            </p>
            <p className="text-sm text-blue-700 dark:text-blue-300">
              {formatDisplayDate(date)}. Attendance tracking is not required.
            </p>
          </div>
        </div>
      )}
      {!context?.is_holiday && context?.is_non_working_day && (
        <div
          className="flex items-start gap-3 rounded-lg border border-gray-200 bg-gray-50 dark:border-slate-600 dark:bg-slate-700/40 p-4"
          role="status"
        >
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              This is a weekend. Employees are not expected to clock in and are not marked absent.
            </p>
          </div>
        </div>
      )}
      ...
      {!error && (
        <div
          className={`grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 transition-opacity ${refreshing ? 'opacity-70' : ''}`}
        >
          {cards.map((card) => {
              ...
              {(summary?.total_employees ?? 0) === 0 ? (
                <>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    There are no active employees in the selected scope.
                  </p>
                </>
                ...
                <>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    This is a holiday or non-working day, so nobody is marked absent.
                  </p>
                </>
              ) : (
                <>
      ...
      {!error && data && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <Card
            ...
            subtitle="Click a department to filter the table above"
          >
          ...
          <Card
            ...
            subtitle="Stacked per-day counts ending at the selected date"
          >
            {trend.length === 0 ? (
              <p className="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
              ...
              <>
                <div className="flex items-end gap-2 h-44 px-1 overflow-x-auto">
                  {trend.map((day) => {
                    ...
                    return (
                      <div
                        key={day.date}
                        className="flex flex-col items-center flex-1 min-w-[44px]"
                      >
                        ...
                        >
                          {total === 0 ? (
                            <div
                              className="w-full bg-gray-200 dark:bg-slate-600"
                              style={{ height: '6px' }}
                            />
                          ) : (
                            <>
                              {day.present > 0 && (
                                <div
                                  className={`w-full ${TREND_COLORS.present}`}
                                  style={{ height: `${pct(day.present)}%` }}
                                />
                              )}
                              {day.absent > 0 && (
                                <div
                                  className={`w-full ${TREND_COLORS.absent}`}
                                  style={{ height: `${pct(day.absent)}%` }}
                                />
                              )}
                              {day.on_leave > 0 && (
                                <div
                                  className={`w-full ${TREND_COLORS.on_leave}`}
                                  style={{ height: `${pct(day.on_leave)}%` }}
                                />
                              )}
                            </>
                          )}
                        ...
                        <span
                          className={`mt-1 text-xs whitespace-nowrap ${day.is_today ? 'font-bold text-primary-600 dark:text-primary-400' : 'text-gray-500 dark:text-gray-400'}`}
                        >
                          {day.label}
                        </span>
                        {(day.is_holiday || day.is_non_working_day) && (
                          <span className="text-[10px] text-gray-400 dark:text-gray-500">
                            {day.is_holiday ? 'Holiday' : 'Weekend'}
                          </span>
                        )}
                      </div>
                    );
                ...
                <div className="flex flex-wrap items-center gap-4 mt-3 pt-3 border-t dark:border-slate-700">
                  <span className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300">
                    <i className="inline-block h-2.5 w-2.5 rounded-sm bg-green-500" /> Present
                  </span>
                  <span className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300">
                    <i className="inline-block h-2.5 w-2.5 rounded-sm bg-red-500" /> Absent
                  </span>
                  <span className="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-300">
                    <i className="inline-block h-2.5 w-2.5 rounded-sm bg-blue-500" /> On Leave
                  </span>
                </div>
              </>
            )}
              ...
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 text-sm">
                <div>
                  ...
                  <dd className="inline text-gray-900 dark:text-gray-100">
                    {detailData?.employee?.name || detailEmployee.name}
                  </dd>
                </div>
                <div>
                  ...
                  <dd className="inline text-gray-900 dark:text-gray-100">
                    {detailData?.employee?.employee_id || detailEmployee.employee_no}
                  </dd>
                </div>
                <div>
                  <dt className="inline font-medium text-gray-700 dark:text-gray-300">
                  ...
                  <dd className="inline text-gray-900 dark:text-gray-100">
                    {detailData?.employee?.department || detailEmployee.department || 'Unassigned'}
                  </dd>
                </div>
                <div>
                  <dt className="inline font-medium text-gray-700 dark:text-gray-300">Section: </dt>
                  <dd className="inline text-gray-900 dark:text-gray-100">
                    {detailData?.employee?.section || detailEmployee.section || '—'}
                  </dd>
                </div>
                <div>
                  <dt className="inline font-medium text-gray-700 dark:text-gray-300">
                  ...
                  <dd className="inline text-gray-900 dark:text-gray-100">
                    {detailData?.employee?.position || detailEmployee.position || '—'}
                  </dd>
                </div>
                <div>
                  <dt className="inline font-medium text-gray-700 dark:text-gray-300">Contact: </dt>
                  <dd className="inline text-gray-900 dark:text-gray-100">
                    {detailEmployee.phone || detailEmployee.email || '—'}
                  </dd>
                </div>
              </dl>
            </section>
            ...
            <section>
              ...
              {detailLoading ? (
                ...
                <p className="py-6 text-center text-sm text-gray-500 dark:text-gray-400">
                  No attendance history for the last 30 days.
                </p>
              ) : (
                      ...
                      {detailData.history.map((rec) => (
                        <tr key={rec.id}>
                          ...
                          <td className="px-4 py-2">
                            <Badge variant={STATUS_META[rec.status]?.variant || 'default'}>
                              {rec.status_label}
                            </Badge>
                          </td>
                        </tr>
