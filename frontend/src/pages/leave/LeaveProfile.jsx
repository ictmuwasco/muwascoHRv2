...
const getStatusInfo = (status) =>
  STATUS_MAP[status] || { label: status || 'Unknown', variant: 'default' };
...
const LeaveProfile = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const fyName =
        financialYears.find((y) => String(y.id) === String(selectedFyId))?.year_name || 'FY';
      link.href = url;
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div
          className={`grid grid-cols-1 gap-4 ${canViewAll ? 'md:grid-cols-3' : 'md:grid-cols-1'}`}
        >
        ...
        <Card>
          <div className="flex items-start space-x-4 flex-wrap">
            <div className="h-16 w-16 rounded-full bg-primary-600 flex items-center justify-center flex-shrink-0">
              <span className="text-2xl font-bold text-white">{getInitials(selectedEmployee)}</span>
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-2 flex-wrap">
                ...
                <Badge variant="primary">{selectedEmployee.employee_id}</Badge>
                {selectedEmployee.employment_type && (
                  <Badge variant="default">{selectedEmployee.employment_type}</Badge>
                )}
              </div>
      ...
      {balanceCards.length > 0 && (
        <Card
          ...
          subtitle="Allocated, brought forward, used and remaining days per leave type"
        >
            ...
            {balanceCards.map((b) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge
                    variant={b.remaining > 0 ? 'success' : b.remaining < 0 ? 'danger' : 'default'}
                  >
                ...
                {applications.map((app) => {
                  ...
                  if (isApproved) {
                    if (Number(app.leave_type_id) === 9) {
                      ...
                      if (primaryDays > 0)
                        impactParts.push(
                          `−${fmtDays(primaryDays)} ${app.leave_type_name || 'primary'}`,
                        );
                            ...
                            <div className="space-y-4">
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                ...
                                <div>
                                  ...
                                  <span className="font-medium text-gray-900 dark:text-gray-100">
                                    {fmtDays(app.days_requested)} days
                                  </span>
                                </div>
                                <div>
                                  <span className="text-gray-500 dark:text-gray-400 block text-xs">
                              ...
                              {app.status === 'rejected' && app.rejection_reason && (
                                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md p-3">
                                  <span className="text-red-600 dark:text-red-400 block text-xs mb-1">
                                    Rejection Reason
                                  </span>
