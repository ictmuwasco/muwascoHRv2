...
const MyMeetings = () => {
  ...
  const fetchMyMeetings = async () => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || err.response?.data?.error || 'Failed to load meetings';
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const handleConfirm = async (meetingId: number) => {
    ...
    setError('');
    try {
      const response = await api.post<{ response_status: string; responded_at: string | null }>(
        `/meetings/${meetingId}/confirm`,
      );
      const invitation = response.data?.data as
        { response_status: string; responded_at: string | null } | undefined;
      ...
      const msg =
        err.response?.data?.message || err.response?.data?.error || 'Unable to accept meeting';
      setError(msg);
  ...
  const handleDecline = async (meetingId: number) => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || err.response?.data?.error || 'Unable to decline meeting';
      setError(msg);
      // Refetch to ensure frontend state matches the database
      await fetchMyMeetings();
    } finally {
  ...
  const getAttendanceBadge = (status: string) => {
    ...
    return (
      <Badge variant={variants[status] || 'default'}>
        {(status || 'not_marked').replace('_', ' ').charAt(0).toUpperCase() +
          (status || 'not_marked').replace('_', ' ').slice(1)}
      </Badge>
    );
  };
  ...
  const getFilteredMeetings = useCallback(() => {
    switch (activeTab) {
      ...
      case 'confirmed':
        ...
        return meetings.filter(
          (m) => m.invitation?.response_status === 'accepted' && !isPastMeeting(m),
        );
      case 'past':
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, row: MeetingInvitation) =>
        getResponseBadge(row.invitation?.response_status || 'pending'),
    },
    {
      key: 'id',
      label: 'Actions',
            ...
            {selectedMeeting.status === 'completed' && (
              <div>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setMinutesMeeting(selectedMeeting)}
                >
            ...
            <div>
              ...
              {selectedMeeting.invitations && selectedMeeting.invitations.length > 0 ? (
                      ...
                      {selectedMeeting.invitations.map((inv) => (
                        <tr key={inv.id}>
                          ...
                          <td className="px-4 py-2">
                            {getResponseBadge(inv.response_status || 'pending')}
                          </td>
                          <td className="px-4 py-2">
                            {getAttendanceBadge(inv.attendance_status || 'not_marked')}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  No employees have been invited to this meeting.
                </p>
              )}
            </div>
          </div>
