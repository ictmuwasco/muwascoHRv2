...
type DateFilterValue =
  'all' | 'today' | 'tomorrow' | 'this_week' | 'next_week' | 'this_month' | 'custom';
...
const MeetingsDashboard = () => {
  ...
  const filteredMeetings = useMemo(() => {
    ...
    if (filters.search.trim()) {
      ...
      result = result.filter((m) => {
        ...
        return (
          title.includes(q) || organizer.includes(q) || location.includes(q) || idStr.includes(q)
        );
      });
    }
  ...
  const hasActiveFilters = () => {
    const hasCustomDateRange =
      filters.dateFilter === 'custom' && (filters.dateFrom !== '' || filters.dateTo !== '');
    return (
      filters.status !== '' ||
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-800 rounded-xl border border-primary-600 dark:border-slate-700 shadow-md p-6 animate-pulse"
            >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400">
              Overview of all scheduled meetings and attendance
            </p>
          </div>
        </div>
        <Card>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Meetings" subtitle="View, search, filter, manage and export all meetings.">
        {/* Toolbar */}
            ...
            <div className="mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex items-center space-x-2">
                <span className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {Math.min(currentPage * perPage, sortedMeetings.length)} of{' '}
