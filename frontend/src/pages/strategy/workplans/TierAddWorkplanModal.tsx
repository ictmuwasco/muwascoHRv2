...
export default function TierAddWorkplanModal({
  ...
  const submit = async () => {
    if (!sourceId) {
      onError('Select the source activity added by management first.');
      return;
    }
    if (rows.length === 0) {
      onError('Add at least one activity.');
      return;
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!row.objective.trim()) {
        onError(`Activity #${i + 1}: the activity description is required.`);
        return;
      }
      if (row.cycle_ids.length === 0) {
        onError(`Activity #${i + 1}: select at least one appraisal cycle (quarter).`);
        return;
      }
    }
    ...
    try {
      const items = rows.map((row) => {
        ...
        if (kind === 'section') {
          ...
          if (hasSubsections && row.subsection_id)
            payload.subsection_id = Number(row.subsection_id);
        }
        return payload;
      });
      ...
      onSaved(
        `${items.length} activit${items.length === 1 ? 'y' : 'ies'} added under the selected source.`,
      );
      onClose();
    } catch (err: any) {
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={kind === 'section' ? 'Add Section Workplan' : 'Add Subsection Workplan'}
      size="xl"
    >
      <div className="space-y-4">
        <div>
          ...
          {sources.length === 0 ? (
            <p className="text-sm text-amber-600 dark:text-amber-400 rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 px-3 py-2">
              No management-added activities are available yet. Wait for your supervisor to cascade
              work to your unit.
            </p>
          ) : (
            <select
          ...
          <p className="text-xs text-gray-400 mt-1">
            New activities below are created under this source and still roll their progress up the
            cascade.
          </p>
        </div>
        ...
        {rows.map((row, idx) => (
          <div
            key={idx}
            className="rounded-lg border border-gray-200 dark:border-slate-700 p-3 space-y-3 bg-gray-50/60 dark:bg-slate-900/40"
          >
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-2 rounded-md border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2 min-h-[42px] items-center">
                {allCycles.length === 0 ? (
                  <span className="text-sm text-gray-400">
                    No appraisal cycles yet — ask HR Admin to create quarterly cycles.
                  </span>
                ) : (
                  allCycles.map((c) => (
                    <label
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={row.cycle_ids.includes(c.id)}
                        onChange={(e) =>
                          updateRow(idx, {
        ...
        <div className="flex justify-end gap-2 pt-2 border-t dark:border-slate-700">
          ...
          <Button onClick={submit} disabled={saving || sources.length === 0}>
            {saving
              ? 'Saving…'
              : `Save ${rows.length || ''} activit${rows.length === 1 ? 'y' : 'ies'}`}
          </Button>
        </div>
      </div>
    </Modal>
