...
function StatCard({
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
      ...
      <p className={`mt-1 text-2xl font-bold ${tone || 'text-gray-900 dark:text-gray-100'}`}>
        {value}
      </p>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
        ...
        {fy && (
          <span className="text-gray-500 dark:text-gray-400">
            Workplan period:{' '}
            <strong className="text-gray-700 dark:text-gray-200">{fy.year_name}</strong> (
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        <div className="rounded-lg border border-primary-200 dark:border-primary-900/50 bg-primary-50 dark:bg-primary-900/20 p-4 col-span-2 md:col-span-1">
          ...
          <p className="mt-1 text-2xl font-bold text-primary-700 dark:text-primary-200">
            {t.completion_rate}%
          </p>
          <div className="mt-2 h-1.5 rounded-full bg-primary-100 dark:bg-primary-900/60 overflow-hidden">
            <div
              className="h-full bg-primary-600"
              style={{ width: `${Math.min(100, t.completion_rate)}%` }}
            />
          </div>
          <p className="mt-1 text-xs text-primary-600/80 dark:text-primary-300/70">
            {t.total_activities} activities
          </p>
        </div>
        <StatCard
          label="Completed"
