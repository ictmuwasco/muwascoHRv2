...
export default function BulkWorkplanModal({
  ...
  const submit = async () => {
    ...
    if (!cid) {
      onError('Select the departmental performance contract.');
      return;
    }
    if (rows.length === 0) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!row.objective.trim()) {
        onError(`Activity #${i + 1}: description is required.`);
        return;
      }
      if (!row.section_id) {
        onError(`Activity #${i + 1}: choose the responsible section.`);
        return;
      }
      if (row.cycle_ids.length === 0) {
        onError(`Activity #${i + 1}: select at least one appraisal cycle.`);
        return;
      }
      items.push({
    ...
    try {
      ...
      onSaved(
        `${items.length} activit${items.length === 1 ? 'y' : 'ies'} saved under the selected contract.`,
      );
      onClose();
    } catch (err: any) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateRow(idx, { section_id: e.target.value, subsection_id: '' })
                  }
                >
