  ...
  return (
    ...
    >
      ...
      <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
        ...
        {(node as any).planned_end_date && (
          <span>Due {fmtDate((node as any).planned_end_date)}</span>
        )}
  ...
  return (
    <ul
      className={
        depth === 0
          ? 'space-y-2'
          : 'mt-2 ml-4 space-y-2 border-l-2 border-dashed border-gray-200 dark:border-slate-600 pl-3'
      }
    >
      {nodes.map((child) => (
        <li key={child.id}>
...
export default function TraceabilityPanel({ objectiveId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    setError('');
    workplanService
      .traceability(objectiveId)
      .then((res) => {
        if (!cancelled) setData(res.data ?? null);
      })
      .catch((err: any) => {
        if (!cancelled) setError(err.response?.data?.message || 'Failed to load traceability.');
      })
      .finally(() => {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Activity Lineage & Traceability"
      size="xl"
    >
      {loading && (
              ...
              {data.ancestors.map((a) => (
                <div key={a.id}>
                  ...
                  <div className="flex justify-center py-0.5">
                    <ArrowDown className="h-4 w-4 text-gray-300 dark:text-slate-500" />
                  </div>
                </div>
              ))}
            </div>
