...
type FormState =
  { open: false } | { open: true; mode: 'add' } | { open: true; mode: 'edit'; record: Contract };
...
export default function PerformanceContracts() {
  ...
  const confirmDelete = (row: Contract) => {
    if (
      !window.confirm(
        `Delete "${row.name}"? Contracts with workplan objectives or KPIs are protected.`,
      )
    )
      return;
    runAction(() => apiClient.delete(`/performance-contracts/${row.id}`), `"${row.name}" deleted.`);
  };
  ...
  if (!payload) {
    return (
      <Card title="Performance Contracts">
    ...
    <div className="space-y-6">
      ...
      {contracts.length === 0 ? (
        ...
        [...planGroups.entries()].map(([planId, planContracts]) => {
          ...
          return (
            <div
              key={planId}
              className="bg-white dark:bg-slate-800 rounded-xl border border-primary-600 dark:border-slate-700 shadow-md overflow-hidden"
            >
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                  <thead className="bg-gray-50 dark:bg-slate-900">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-[17%]">
                        Organisation's Goal
                      </th>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-[19%]">
                        Departmental Performance Contract
                      </th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-[11%]">
                        KPI
                      </th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase w-[8%]">
                        Created
                      </th>
                      <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase w-[8%]">
                        Actions
                      </th>
                    </tr>
                  </thead>
                        ...
                        {rows.map((row, idx) => {
                          ...
                          return (
                            ...
                            >
                                ...
                                {isFirstOfGoal && (
                                  <div className="flex items-center gap-2">
                                    <Badge variant="success">
                                      {rows[0].goal_name ?? 'Unnamed Perspective'}
                                    </Badge>
                              ...
                              <td className="px-4 py-3 align-top">
                                <Badge variant="primary">{row.financial_year_name ?? '—'}</Badge>
                              </td>
                              <td className="px-4 py-3 align-top text-sm text-gray-900 dark:text-gray-100">
                                {row.name}
                              </td>
                              <td className="px-4 py-3 align-top text-sm text-gray-500 whitespace-pre-wrap">
                                {row.kra || '—'}
                              </td>
      ...
      {detail && (
        <Modal
          ...
          size="xl"
          title={`Contract — ${detail.name}`}
        >
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <p>
                <span className="font-medium">Strategic plan:</span>{' '}
                {detail.strategic_plan_name ?? 'N/A'}
              </p>
              <p>
                <span className="font-medium">Goal perspective:</span> {detail.goal_name ?? 'N/A'}
              </p>
              <p>
                <span className="font-medium">Organisation's goal:</span>{' '}
                {detail.target_name ?? '—'}
              </p>
              <p>
                <span className="font-medium">Department:</span> {detail.department_name ?? '—'}
              </p>
              <p>
                <span className="font-medium">Financial year:</span>{' '}
                {detail.financial_year_name ?? '—'}
              </p>
              <p>
                <span className="font-medium">KPI (KRA):</span> {detail.kra || 'Not recorded'}
              </p>
            </div>
            <div className="border-t pt-3 dark:border-slate-700">
              ...
              {detailWorkplans.length === 0 ? (
                <p className="text-sm text-gray-500">
                  No workplan objectives captured for this contract yet.
                </p>
              ) : (
                <ul className="space-y-2">
                  {detailWorkplans.map((w) => (
                    <li
                      key={w.id}
                      className="rounded-lg border border-gray-200 dark:border-slate-700 px-3 py-2 text-sm"
                    >
...
function ContractFormModal({
  ...
  const perspectives =
    planId === '' ? goals : goals.filter((g) => g.strategic_plan_id === Number(planId));
  const orgGoals = goalId === '' ? [] : targets.filter((t) => t.goal_id === Number(goalId));
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select
              className="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm"
              value={planId}
              onChange={(e) => changePlan(e.target.value ? Number(e.target.value) : '')}
              required
            >
          ...
          <div>
            ...
            <select
              className="w-full rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-sm"
              value={fyId}
              onChange={(e) => setFyId(e.target.value ? Number(e.target.value) : '')}
              required
            >
          ...
          <div>
            ...
            >
              <option value="">
                {planId === '' ? 'Select a strategic plan first' : 'Select Goal Perspective'}
              </option>
              {perspectives.map((g) => (
        ...
        <div>
          ...
          >
            <option value="">
              {goalId === '' ? 'Select a goal perspective first' : "Select Organisation's Goal"}
            </option>
            {orgGoals.map((t) => (
        ...
        <div>
          ...
          <input
            ...
            placeholder="Performance contract name"
            required
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Key result area / indicator for this contract"
            required
          />
        </div>
