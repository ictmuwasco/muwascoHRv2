...
export const attendanceReportService = {
  ...
  byDepartment: async (params?: Record<string, any>): Promise<any> => {
    const res = await apiClient.get<ApiResponse<any>>('/reports/attendance/by-department', {
      params,
    });
    return res.data?.data;
  },
  ...
  lateArrivals: async (params?: Record<string, any>): Promise<any> => {
    const res = await apiClient.get<ApiResponse<any>>('/reports/attendance/late-arrivals', {
      params,
    });
    return res.data?.data;
  },
  ...
  workingHours: async (params?: Record<string, any>): Promise<any> => {
    const res = await apiClient.get<ApiResponse<any>>('/reports/attendance/working-hours', {
      params,
    });
    return res.data?.data;
  },
