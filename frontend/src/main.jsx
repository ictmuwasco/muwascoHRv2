  ...
  .then(({ reportClientError }) => {
    window.addEventListener('unhandledrejection', (event) => {
      ...
      reportClientError({
        ...
        message: `Unhandled promise rejection: ${reason?.message || String(reason ?? '')}`.slice(
          0,
          1000,
        ),
    ...
    window.addEventListener(
      ...
      true,
    ); // capture phase so resource errors on media/script tags are caught too
  })
  .catch(() => {
    /* reporter unavailable - nothing else we can do */
  });
