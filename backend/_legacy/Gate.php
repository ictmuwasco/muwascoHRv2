...
class Gate
{
    ...
    public function allows(string $ability, array $arguments = []): bool
    {
        ...
        // Default to the single authorization engine. The authenticated user
        // ID is passed (never the role string — roles are derived inside the
        // service from trusted context). Ability notation "module:action" is
        // also supported via the module:action arguments.
        $authService = AuthorizationService::getInstance();
        $resource = is_string($arguments[0] ?? null) ? $arguments[0] : '';
        $action = is_string($arguments[1] ?? null) ? $arguments[1] : 'view';

        return $authService->hasPermission((int) $userId, $resource, $action);
    }
